...
use tokio::io::{self, AsyncWriteExt, AsyncBufReadExt, Error};
        ...
        for section in sections {
            if first {
                if section != b"YUV4MPEG2" {
                    println!("{}", header.len());
                    return Err(Error::new(ErrorKind::InvalidData, format!("Wrong magic word {}",
            ...
            match section[0] {
                b'W' => width = tail.parse().map_err(|err: ParseIntError|Error::new(ErrorKind::InvalidData, err)),
                b'H' => height = tail.parse().map_err(|err: ParseIntError|Error::new(ErrorKind::InvalidData, err)),
                ...
                b'C' => {
                    match tail {
                        ...
                        _ => return Err(Error::new(ErrorKind::InvalidData, format!("Unsupported colorspace '{}'", tail)))
                    }
                }
                b'X' => {} // This is a comment parameter, not supported,
                _ => return Err(Error::new(ErrorKind::InvalidData, format!("Unknown parameter '{}'", std::str::from_utf8(section).expect("Section should parse"))))
            }
        }
...
mod tests {
    ...
    async fn test_read() {
        ...
        let header = VideoHeader::read(&mut Cursor::new(vec)).await.expect("should succeed");
