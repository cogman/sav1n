...
impl<'a> Frame {
    pub fn new(data_len: usize, num: u64) -> Self {
        Frame {
            ...
            num,
            data: unsafe {
                std::alloc::alloc(Layout::array::<u8>(data_len).expect("Length too long"))
            },
        }
    }
...
impl Clone for Frame {
    fn clone(&self) -> Self {
        let data_ptr;
        unsafe {
            data_ptr = std::alloc::alloc(Layout::array::<u8>(self.data_len).expect("weird layout"));
    ...
    fn clone_from(&mut self, source: &Self) {
        ...
        unsafe { std::ptr::copy(source.data, self.data, source.data_len) }
    }
}
    ...
    fn drop(&mut self) {
        if self.droppable {
            unsafe {
                std::alloc::dealloc(
                    self.data,
                    Layout::array::<u8>(self.data_len).expect("layout"),
                )
            }
        }
    }
